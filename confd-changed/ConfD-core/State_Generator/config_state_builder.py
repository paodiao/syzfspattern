...
def _parse_int_range(val):
    """Strip trailing '+' or other annotations from range values."""
...
def verify_config(my_config, constraint_data):
    ...
    for a in my_config.arg:
        ...
        if entry.get("takes_value") == "yes":
            ...
            if vmax is not None and _parse_int_range(vmax) < _parse_int_range(my_config.arg[a]):
                return False
            if vmin is not None and _parse_int_range(vmin) > _parse_int_range(my_config.arg[a]):
                return False
...
def generate(my_config, constraint_data, target_num, final_states, try_list, defaults, id_to_name=None):
    global states_made
    global depth
    ...
    for id in try_list:
        ...
        if existing is not None:
            ...
            if defaults is not None and defaults.get(name) is not None:
                # Has a default: toggle away from it
                ...
                if tv == "no" or tv == "flag_only":
                    ...
                    vmin = _parse_int_range(entry["value_range_min"])
                    vmax = entry.get("value_range_max")
                    if vmax is not None:
                        vmax = _parse_int_range(vmax)
                        ...
                        temp.arg[name] = vmin
                        temp2.arg[name] = vmax
                        temp3.arg[name] = nextPow2(vmin + 1)
                    else:
                        temp.arg[name] = vmin
                else:
                ...
                # No default: already explored, skip
                continue
        else:
        ...
        for temp_config in new_configs:
            ...
            if not seen:
                ...
                # Determine what to try next from dependencies
                next_list = []
                ...
                generate(temp_config, constraint_data, target_num, final_states, next_list, defaults, id_to_name)
...
def main(argv):
    ...
    if len(sys.argv) != 3:
        print("Usage: python3 config_state_builder.py <max_depth> <max_states>")
    ...
    generate(copy.deepcopy(my_config), constraint_data, max_final_states, final_states, all_ids, defaults, id_to_name)
