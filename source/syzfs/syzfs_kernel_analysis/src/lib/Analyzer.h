...
struct ConstraintInfo {
	...
	set<pair<uint64_t, string>> handlerConstSet;
  	...
  	//set<pair<string, string>> fsMountOptionSwitchSet;
	set<pair<string, string>> fsMountOptionIfSet;
	string relateModule;
};
...
// One-layer propagation value with cmp_info (for assignint/assignenum)
typedef std::pair<std::pair<std::string, std::pair<uint32_t, std::string>>, std::pair<std::string, uint64_t>> OneLayerVal;
typedef std::set<OneLayerVal> OneLayerPairSet;

// Superblock mapping entry (from superblock_mapping_{fs}.json)
struct SuperblockMappingEntry {
    std::string confd_param;
    std::string confd_variable;
    std::string disk_field;
    std::string bitmask_hex;
    int64_t bitmask_value;
    std::string kernel_func;
    std::string kernel_mem_field;
    std::string transform;
    std::string formula;
    std::string embedded_struct_type;     // LLVM struct type (e.g. "struct.ext4_super_block")
    int64_t embedded_field_offset;        // byte offset in that struct, -1 if null
};
...
struct GlobalContext {
	...
	map<string, std::map<std::pair<std::string, std::pair<uint32_t, string>>, OneLayerPairSet>> fs2options2onelayer;
	map<string, std::vector<SuperblockMappingEntry>> superblock_mappings;
};
