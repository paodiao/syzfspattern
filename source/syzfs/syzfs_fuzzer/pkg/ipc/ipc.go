...
func MakeEnv(config *Config, pid int) (*Env, error) {
	...
	env := &Env{
		...
		inFile:  inf,
		outFile: outf,
		bin:     append(append(strings.Split(config.Executor, " "), "exec"), config.FSName, strconv.Itoa(config.FSImageNum), strconv.Itoa(config.FSMntCmdNum), strconv.Itoa(config.FSMntCmdCur)),
		pid:     pid,
		config:  config,
	}
	if len(env.bin) == 0 {
