...
func (inst *inst) testInstance() error {
	...
	cmd := OldFuzzerCmd(fuzzerBin, executorBin, targets.TestOS, inst.cfg.TargetOS, inst.cfg.TargetArch, fwdAddr,
		inst.cfg.Sandbox, 0, inst.cfg.Cover, true, inst.optionalFlags, inst.cfg.Timeouts.Slowdown, inst.cfg.FSName, inst.cfg.FSImageNum, inst.cfg.FSMntCmdNum, inst.cfg.FSMntCmdCur)
...
func FuzzerCmd(args *FuzzerCmdArgs) string {
	...
	return fmt.Sprintf("%v -executor=%v -name=%v -arch=%v%v -manager=%v -sandbox=%v"+
		...
		" -FSName=%v -FSImageNum=%v -FSMntCmdNum=%v -FSMntCmdCur=%v",
		...
		args.FSName, args.FSImageNum, args.FSMntCmdNum, args.FSMntCmdCur)
}

func OldFuzzerCmd(fuzzer, executor, name, OS, arch, fwdAddr, sandbox string, procs int,
	cover, test, optionalFlags bool, slowdown int, fsname string, fsimagenum int, fsmntcmdnum int, fsmntcmdcur int) string {
	...
	return FuzzerCmd(&FuzzerCmdArgs{Fuzzer: fuzzer, Executor: executor, Name: name,
		...
		Optional: optional, FSName: fsname, FSImageNum: fsimagenum, FSMntCmdNum: fsmntcmdnum, FSMntCmdCur: fsmntcmdcur})
}
