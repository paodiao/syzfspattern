...
// Handshake-stage mount coverage (collected once in receive_handshake,
// consumed in execute_one).
static bool handshake_cov_pending;
static uint32 handshake_cov_size;
static uint32 handshake_cov_dist[DIST_SIZE];
static char handshake_cov_kcov[kCoverSize * sizeof(uint64)];
...
uint64 fs_image_num = 0, fs_mnt_cmd_num = 0, fs_mnt_cmd_cur = 0
...
int main(int argc, char** argv)
{
	...
	fs_name = argv[2];
	fs_image_num = atoi(argv[3]);
  	fs_mnt_cmd_num = atoi(argv[4]);
	fs_mnt_cmd_cur = atoi(argv[5]);
...
void receive_handshake()
{
	// mount dfs and chdir
    char cmdbuf[120];
    snprintf(cmdbuf, 119, "/root/%s-mount.sh %lld %lld %lld 2>&1", fs_name, fs_image_num,
             fs_mnt_cmd_num, fs_mnt_cmd_cur);

    if (flag_coverage) {
        cover_reset(&extra_cov);
        cover_enable(&extra_cov, false, false);
    }
    ...
    if (cmd != NULL) {
      ...
      while (fgets(result, sizeof(result), cmd) != NULL)
        fprintf(stderr, "---- executor mount FS: %s\n", result);
      pclose(cmd);
    }

    if (flag_coverage) {
        cover_collect(&extra_cov);
        ioctl(extra_cov.fd, KCOV_DISABLE);
        if (extra_cov.size > 0 && extra_cov.size < (kCoverSize / sizeof(uint64))) {
            handshake_cov_size = extra_cov.size;
            memcpy(handshake_cov_dist, extra_cov.dist_area, DIST_SIZE * sizeof(uint32));
            uint32 word_size = is_kernel_64_bit ? sizeof(uint64) : sizeof(uint32);
            uint32 copy_bytes = (1 + extra_cov.size) * word_size;
            memcpy(handshake_cov_kcov, extra_cov.kcov_area, copy_bytes);
            handshake_cov_pending = true;
        }
    }

	// change to work dir
  	char cwdbuf[240] = "/root/mntimg";
  	if (chdir(cwdbuf)) {
    	char outmsg[1024];
    	snprintf(outmsg, 1023, "failed to chdir: %s\n", cwdbuf);
    	fail(outmsg);
  	}
...
void execute_one()
{
	...
	if (handshake_cov_pending && cover_collection_required()) {
		cover_t tmp;
		tmp.size = handshake_cov_size;
		tmp.dist_area = handshake_cov_dist;
		tmp.kcov_area = handshake_cov_kcov;
		tmp.data_offset = (is_kernel_64_bit ? sizeof(uint64) : sizeof(uint32));
		tmp.pc_offset = 0;
		write_output(kOutMagic);
		write_output(-1); // extraReplyIndex
		write_output(-1); // call_num
		write_output(999); // errno
		write_output(0); // flags
		uint32* signal_count_pos = write_output(0);
		uint32* cover_count_pos = write_output(0);
		write_output(0); // comps
		write_output(tmp.dist_area[0]); // distance
		uint32* hit_num_pos = write_output(0);
		if (is_kernel_64_bit)
			write_coverage_signal<uint64>(&tmp, signal_count_pos, cover_count_pos, hit_num_pos);
		else
			write_coverage_signal<uint32>(&tmp, signal_count_pos, cover_count_pos, hit_num_pos);
		handshake_cov_pending = false;
	}
#endif
