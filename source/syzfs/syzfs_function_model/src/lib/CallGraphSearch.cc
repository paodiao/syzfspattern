...
void GenerateFunctionCandidate(GlobalContext* Ctx, string syscall, Function* start, unordered_map<Function*, bool> terminators, int maxDepth, map<Function*, vector<InfoItem*>>& functionitems) {
    visited.clear();
    set<Function*> visitfunc;
    Ctx->SyscallHandlerCandidates[syscall] = vector<Function*>();
    for(auto& term : terminators) {
        if(term.second == true) {
            Function* fterm = term.first;
            if(functionitems[fterm].size() != 0) {
                InfoItem* fitem = functionitems[fterm][0];
                if(fitem->ItemType == FILESYSTEM) {
                    Ctx->SyscallHandlerCandidates[syscall].push_back(fterm);
                    visitfunc.insert(fterm);
                    //terminators[fterm] = false;
                }
            }
        }
    }
    
    for(auto& vf : visitfunc) {
        terminators[vf] = false;
    }
