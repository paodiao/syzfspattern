...
cl::opt<string> SourceLocation(
        "source-location", cl::desc("Input the target source code location"), cl::init("")
);
...
int main(int argc, char **argv) {
  ...
  GlobalCtx.CaseSrcDir = std::string(SourceLocation);
	// Main workflow
  ...
  raw_fd_ostream fsFlags(StringRef("./fsflags"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsEnums(StringRef("./fsenums"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsOptions(StringRef("./fsoptions"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsOptionsonelayer(StringRef("./fsoptionsonelayer"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsInts(StringRef("./fsints"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsinitinodefuncs(StringRef("./fsinitinodefuncs"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsoldparsefuncs(StringRef("./fsoldparsemountoptionsfuncs"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsfunctiontableallocfile(StringRef("./fsfunctiontablealloc"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsentryfunctiontable(StringRef("./fsentryfunctiontable"), OutErrorInfo, sys::fs::CD_CreateAlways);
  raw_fd_ostream fsDepsFile(StringRef("./fsdeps"), OutErrorInfo, sys::fs::CD_CreateAlways);

  for(auto& fsinitinodefunc : GlobalCtx.Fs2InodeInitFuncs) {
    string filesys = fsinitinodefunc.first;
    set<Function*> initfuncs = fsinitinodefunc.second;
    for(auto& initfunc : initfuncs) {
      if(initfunc->hasName()) {
        fsinitinodefuncs << filesys << " " << initfunc->getName().str() << "\n";
      }
    }
  }

  for(auto& fsoldparsefunc : GlobalCtx.Fs2OldParseMountOptionsFuncs) {
    string filesys = fsoldparsefunc.first;
    std::set<std::pair<Function*, CallInst*>> oldparsefuncs = fsoldparsefunc.second;
    for(auto& oldparsefuncandcall : oldparsefuncs) {
      Function* oldparsefunc = oldparsefuncandcall.first;
      if(oldparsefunc->hasName()) {
        fsoldparsefuncs << filesys << " " << oldparsefunc->getName().str() << "\n";
      }
    }
  }
  
  for(auto& fsfunctiontablealloc : GlobalCtx.fs2functiontablealloc) {
    string filesys = fsfunctiontablealloc.first;
    std::map<std::string, std::set<string>> functiontableallocs = fsfunctiontablealloc.second;
    for(auto& functiontablealloc : functiontableallocs) {
      string functiontablename = functiontablealloc.first;
      set<string> functiontableconstraints = functiontablealloc.second;
      for(auto& functiontableconstraint : functiontableconstraints) {
        fsfunctiontableallocfile << filesys << " " << functiontablename << " " << functiontableconstraint << "\n";
      }
    }
  }

  // L2 mount option dependency output
  for (auto& fsdep : GlobalCtx.fs2deps) {
    string fsname = fsdep.first;
    FsDeps& deps = fsdep.second;

    for (auto& sc : deps.self) {
      fsDepsFile << fsname << " self " << sc.opt_token << " " << sc.opt_name
                 << " " << sc.dep_type;
      if (sc.dep_type == "range")
        fsDepsFile << " " << sc.min_val << " " << sc.max_val;
      else if (sc.dep_type == "discrete") {
        fsDepsFile << " " << sc.valid_values.size();
        for (auto v : sc.valid_values) fsDepsFile << " " << v;
      }
      fsDepsFile << "\n";
    }
    for (auto& cd : deps.cross) {
      fsDepsFile << fsname << " cross " << cd.opt_A << " " << cd.opt_B
                 << " " << cd.opt_A_name << " " << cd.opt_B_name
                 << " " << cd.relation << " " << cd.src_file << " " << cd.src_line << "\n";
    }
  }

  for(auto& fsentryfunctable: GlobalCtx.Fs2EntryFunc2FuncTable) {
    string filesys = fsentryfunctable.first;
    std::map<std::string, std::set<string>> entryfunctables = fsentryfunctable.second;
    for(auto& entryfunctable : entryfunctables) {
      string entryname = entryfunctable.first;
      set<string> functiontables = entryfunctable.second;
      for(auto& functable : functiontables) {
        fsentryfunctiontable << filesys << " " << entryname << " " << functable << "\n";
      }
    }
  }

  for(auto& fsparamflag : GlobalCtx.fsparamsflag) {
    string fsstr = fsparamflag.first;
    std::map<int, string> flagnumstrs = fsparamflag.second;
    for(auto& numstr : flagnumstrs) {
      int optnum = numstr.first;
      string optname = numstr.second;
      //outs() << "test output fsflags: " << fsstr << " " << optnum << " " << optname << "\n";
      fsFlags << fsstr << " " << optnum << " " << optname << "\n";
    }
  }

  for(auto& fsparamint : GlobalCtx.fsparamsint) {
    string fsstr = fsparamint.first;
    std::map<int, std::pair<std::string, int>> intnumstrs = fsparamint.second;
    for(auto& numstr : intnumstrs) {
      int optnum = numstr.first;
      string optname = numstr.second.first;
      int radix = numstr.second.second;
      fsInts << fsstr << " " << optnum << " " << optname << " " << radix << "\n";
    }
  }

  for(auto& fsparamenum : GlobalCtx.fsparamsenum) {
    string fsstr = fsparamenum.first;
    std::map<std::pair<int, std::string>, std::map<int, std::string>> enumnumstroptions = fsparamenum.second;
    for(auto& enumnumstropt : enumnumstroptions) {
      std::pair<int, string> enumnumstr = enumnumstropt.first;
      int enumnum = enumnumstr.first;
      string enumstr = enumnumstr.second;
      std::map<int, std::string> enumoptions = enumnumstropt.second;
      for(auto& enumopt : enumoptions) {
        int enumoptnum = enumopt.first;
        string enumoptstr = enumopt.second;
        //outs() << "test output fsenums: " << fsstr << " " << enumnum << " " << enumstr << " " << enumoptnum << " " << enumoptstr << "\n";
        fsEnums << fsstr << " " << enumnum << " " << enumstr << " " << enumoptnum << " " << enumoptstr << "\n";
      }
    }
  }

  for(auto& fsopts : GlobalCtx.fs2options) {
    string fsstr = fsopts.first;
    std::map<string, std::set<std::pair<std::pair<uint32_t, string>, std::pair<int, int>>>> optsmap = fsopts.second;
    for(auto& optsitem : optsmap) {
      string optstructfieldstr = optsitem.first;
      std::set<std::pair<std::pair<uint32_t, string>, std::pair<int, int>>> FlagAndOptNum = optsitem.second;
      for(auto& flagopt : FlagAndOptNum) {
        std::pair<uint32_t, string> optflagandconf = flagopt.first;
        std::pair<int, int> optnumandenum = flagopt.second;
        uint32_t optflag = optflagandconf.first;
        string confg = optflagandconf.second;
        int optnum = optnumandenum.first;
        int enumnum = optnumandenum.second;
        //outs() << "test output fsoptions: "<< fsstr << " " << optstructfieldstr << " " << optflag << " " << confg << " " << optnum << " " << enumnum << "\n";
        fsOptions << fsstr << " " << optstructfieldstr << " " << optflag << " " << confg << " " << optnum << " " << enumnum << "\n";
      }
    }
  }

  for(auto& fsoptprop : GlobalCtx.fs2options2onelayer) {
    string fsstr = fsoptprop.first;
    std::map<std::pair<std::string, std::pair<uint32_t, std::string>>, OneLayerPairSet> optspropmap = fsoptprop.second;
    for(auto& optptop : optspropmap) {
      std::pair<std::string, std::pair<uint32_t, std::string>> onelayerstructfield = optptop.first;
      string onelayerstructfieldstr = onelayerstructfield.first;
      std::pair<uint32_t, std::string> onelayerstructfieldstrflagandcfg = onelayerstructfield.second;
      uint32_t onelayerstructfieldstrflag = onelayerstructfieldstrflagandcfg.first;
      string onelayerstructfieldstrcfg = onelayerstructfieldstrflagandcfg.second;
      OneLayerPairSet optstructfieldflags = optptop.second;
      for(auto& olv : optstructfieldflags) {
        auto& innerKey = olv.first;
        auto& cmpInfo = olv.second;
        string optstfld = innerKey.first;
        std::pair<uint32_t, std::string> optflagandconf = innerKey.second;
        uint32_t optflag = optflagandconf.first;
        string conf = optflagandconf.second;
        string cmpOp = cmpInfo.first;
        uint64_t cmpVal = cmpInfo.second;
        fsOptionsonelayer << fsstr << " " << onelayerstructfieldstr << " " << onelayerstructfieldstrflag << " " << onelayerstructfieldstrcfg << " " << optstfld << " " << optflag << " " << conf << " " << cmpOp << " " << cmpVal << "\n";
      }
    }
  }
  ...
  for (auto item: TerminatorHandlerCandidates) {
    ...
    GenerateFunctionCandidate(&GlobalCtx, syscall, syscallFunction, item.second, GlobalSyscallRecrusiveCount[syscall], funcInfoItemMap);
