...
class FilesystemExtractorPass : public IterativeModulePass {
	...
	private:
		...
		std::map<int, std::string> getPotentialFlagsinOldParams(GlobalVariable* fsparams, std::map<int, std::pair<std::string, int>>& intParams, std::map<std::pair<int, std::string>, std::map<int, std::string>>& enumParams);
		std::map<std::string, std::set<std::pair<std::pair<uint32_t, string>, std::pair<int,int>>>> collectOldParamsParse(Function* parseF, CallInst* CallToParseF, std::map<int, std::string> flagParams, std::map<int, std::pair<std::string, int>>& intParams, std::set<string>& relatedSt, std::map<int, std::string>& enumCollect);
		std::map<std::string, std::set<string>> collectFunctionTableAlloc(Function* InodeInitF, std::map<std::pair<int, std::string>, std::map<int, std::string>> &sigfsparamsenum, std::map<int, string> &sigfsparamsflag, std::map<std::string, std::set<std::pair<std::pair<uint32_t, string>, std::pair<int, int>>>> &sigfs2options, std::map<std::pair<std::string, std::pair<uint32_t, std::string>>, OneLayerPairSet> &sigfs2options2onelayer);
