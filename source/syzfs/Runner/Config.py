...
def PreparePathVariables():
    ...
    FuzzerDir=os.path.join(ResourceRoot,"syzfs_fuzzer")
    ...
    global FunctionModelDirRoot,FunctionModelBinary,InterfaceDirRoot,getInterfaceDirByCase, getKernelSignatureByCase,getFinalInterfaceParingResultByCase, getFinalFsparamsFlagByCase, getFinalFsparamsEnumByCase, getFinalFs2OptionsByCase, getFinalFs2Options2OnelayerByCase
    FunctionModelDirRoot=os.path.join(ResourceRoot,"syzfs_function_model")
    ...
    getFinalFsparamsFlagByCase=lambda caseIdx: os.path.join(getInterfaceDirByCase(caseIdx),"fsflags")
    getFinalFsparamsEnumByCase=lambda caseIdx: os.path.join(getInterfaceDirByCase(caseIdx),"fsenums")
    getFinalFs2OptionsByCase=lambda caseIdx: os.path.join(getInterfaceDirByCase(caseIdx),"fsoptions")
    getFinalFs2Options2OnelayerByCase=lambda caseIdx: os.path.join(getInterfaceDirByCase(caseIdx),"fsoptionsonelayer")
    ...
    TargetPointAnalysisDirRoot=os.path.join(ResourceRoot,"syzfs_kernel_analysis")
...
def PrepareArgParser():
    ...
    parser=argparse.ArgumentParser(description='This is the runner script for syzfs.')
    ...
    parser.add_argument('-fuzz-rounds',type=int,default=7,help="run rounds for every case",dest="run_rounds")
    parser.add_argument('-uptime',type=int,default=20,help="fuzzing timeout(hours) for every case, default set to 24 ")
