...
if [ "x$OFILE" != x -a "$OFILE" != "$*" ] ; then
    $CLANG -emit-llvm -g -fno-eliminate-unused-debug-types "$@" >/dev/null 2>&1 > /dev/null
    if [ -f "$OFILE" ] ; then
...
exec $CLANG "$@"
    ...
    for datapoint in Config.datapoints:
        ...
        compile_command = f"cd {Config.getSrcDirByCase(caseIdx)} && git checkout -- scripts/Makefile.kcov && make clean && make mrproper && yes | make CC={Config.EmitScriptPath} O={caseBCDir} oldconfig && make CC=\'{Config.EmitScriptPath}\' O={caseBCDir} -j8"
...
make ARCH=x86_64 CC=$CC O=%s -j%s
    '''%(caseSrcDir, Config.ClangPath, tempBuildDir,tempBuildDir, 16)
...
def IsCompilationSuccessful():
    ...
    # return all fail cases, [] if all succeed
def IsCompilationSuccessfulforsrc():
    return [
        datapoint['idx'] for datapoint in Config.datapoints if not IsCompilationSuccessfulByCase(Config.getSrcDirByCase(datapoint['idx']))
    ]