...
def AnalyzeKernelInterface():
    ...
    for datapoint in Config.datapoints:
        ...
        if not os.path.exists(caseKernelSignatureFile):
            generating_cmd=f"cd {caseInterfaceWorkingDir} && {Config.FunctionModelBinary} --verbose-level=4 --source-location={caseSrcDir} {caseSrcDir} 2>&1 | tee log"
...
def AnalyzeTargetPoints():
    ...
    for datapoint in Config.datapoints:
        caseIdx=datapoint['idx']
        caseFinalInterfaceFile=Config.getFinalInterfaceParingResultByCase(caseIdx)
        caseFinalFsFlagFile=Config.getFinalFsparamsFlagByCase(caseIdx)
        caseFinalFsEnumFile=Config.getFinalFsparamsEnumByCase(caseIdx)
        caseFinalFsOptionsFile=Config.getFinalFs2OptionsByCase(caseIdx)
        caseFinalFsOptionsOnelayerFile=Config.getFinalFs2Options2OnelayerByCase(caseIdx)
        caseBitcodeDir=Config.getBitcodeDirByCase(caseIdx)
        caseSrcDir=Config.getSrcDirByCase(caseIdx)
        ...
        analyze_cmd=f"cd {caseKernelAnalysisResDir} && {Config.TargetPointAnalysisBinary} --verbose-level=4 -kernel-interface-file={caseFinalInterfaceFile} -fsparams-flag-file={caseFinalFsFlagFile} -fsparams-enum-file={caseFinalFsEnumFile} -fs2options-file={caseFinalFsOptionsFile} -fs2options2onelayer-file={caseFinalFsOptionsOnelayerFile} -multi-pos-points={Config.getMultiPointsSpecificFile(caseIdx)} {caseSrcDir} 2>&1 | tee log"
