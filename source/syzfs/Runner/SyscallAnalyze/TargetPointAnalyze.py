...
filesystems=["sysfs", "rootfs", "ramfs", "tmpfs", "devtmpfs", "debugfs", "securityfs", "sockfs", "pipefs", "anon_inodefs", "devpts", "ext3", "ext2", "ext4", "hugetlbfs", "vfat", "ecryptfs", "fuseblk", "fuse", "rpc_pipefs", "nfs", "nfs4", "nfsd", "binfmt_misc", "autofs", "xfs", "jfs", "msdos", "ntfs3", "ntfs", "minix", "hfs", "hfsplus", "qnx4", "ufs", "btrfs", "configfs", "ncpfs", "qnx6", "exofs", "befs", "vxfs", "gfs2","gfs2meta", "fusectl", "bfs", "nsfs", "efs", "cifs", "efivarfs", "affs", "tracefs", "bdev", "ocfs2", "ocfs2_dlmfs", "hpfs", "proc", "afs", "reiserfs", "jffs2", "romfs", "aio", "sysv", "v7", "udf", "ceph", "pstore", "adfs", "9p", "hostfs", "squashfs", "cramfs", "iso9660", "coda", "nilfs2", "logfs", "overlay", "f2fs", "omfs", "ubifs", "openpromfs", "bpf", "cgroup", "cgroup2", "cpuset", "mqueue", "aufs", "selinuxfs", "dax", "erofs", "virtiofs", "exfat", "binder", "zonefs", "pvfs2", "incremental-fs", "esdfs"]
...
def PrepareForFuzzing(caseIdx, recommend_syscalls):
    ...
    for xidx in case2csts.keys():
        ...
        Refine_Input_File_Now=f"constinpcase{caseIdx}xidx{xidx}.json"
        Refine_Input_File_Now_Path=os.path.join(TheFuzzerPath,Refine_Input_File_Now)
        if os.path.exists(Refine_Input_File_Now_Path):
            os.remove(Refine_Input_File_Now_Path)

        with open(Refine_Input_File_Now_Path, "w") as fp:
            json.dump(constraints, fp)
        ...
        if str(xidx) in Case2Func.keys():
            ...
            if relativepath[:3]=="fs/":
                ...
                print("relatedFScandidate: ",relatedFScandidate)
                if relatedFScandidate in filesystems:
                    relatedFS=relatedFScandidate
                    print(f"idx {xidx}: target function file: ",relativepath)
